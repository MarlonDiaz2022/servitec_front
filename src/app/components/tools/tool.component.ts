...
export class ToolComponent implements OnInit {
  ...
  if (this.selectedFile) {
       ...
       console.log("No se seleccionó nuevo archivo, usando URL existente (si aplica)");
    } else if (this.editingTool && !this.toolForm.imageUrl) {
    ...
    } else { // Creando y no se seleccionó archivo
      ...
      this.toolservice.deleteTool(id).subscribe(
        ...
        error => {
          console.error('Error al eliminar herramienta', error);
           alert('Error al eliminar herramienta: ' + (error.error?.message || error.message));
        }
      );
